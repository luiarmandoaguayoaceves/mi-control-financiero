// Tests de la lógica financiera pura (node --test, sin dependencias RN).
// Node 24 ejecuta TypeScript nativamente (type stripping); los imports
// relativos usan extensión .ts a propósito.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import type { Account, CreditCard, Fund, InstallmentPurchase, Service, Transaction } from '../src/models/types';
import {
  budgetStatus,
  currentCycleRange,
...
test('msiMonthlyTotal suma las mensualidades activas', () => {
  const items: InstallmentPurchase[] = [
...
test('pago proyectado no se confunde con el saldo total de la tarjeta (MSI futuros)', () => {
  const card: CreditCard = {
    id: 'cc1', name: 'BBVA Azul', bank: 'BBVA', creditLimit: 111700,
    currentBalance: 20000.5, cutDay: 12, currentNoInterestPayment: 0,
    availableCredit: 91699.5, points: 268, active: true,
  };
  assert.ok(card.currentBalance > nextTdcPayment(3177.45, 1482));
  assert.equal(nextTdcPayment(3177.45, 1482), 4659.45);
});
